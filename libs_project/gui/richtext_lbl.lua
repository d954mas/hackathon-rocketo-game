...
function Lbl:set_width(width)
    self.text_setting.width = assert(width)
end
...
function Lbl:set_text(text)
    ...
    if (self.center_v) then
        gui.set_position(self.root_node, vmath.vector3(self.position.x, self.position.y + self.text_metrics.height / 2, 0))
    else
        gui.set_position(self.root_node, self.position)
    end
end

function Lbl:dispose()
    if(self.nodes)then
        for _, node in ipairs(self.nodes) do
            gui.delete_node(node.node)
        end
