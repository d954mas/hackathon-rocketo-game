...
local function position_words(words, line_width, line_height, position, settings)
	...
	for i=1,#words do
		...
		elseif word.image then
			local pos = vmath.vector3(position.x,position.y + (M.DEFAULT_ALIGN == nil and line_height/4 or 0),0)
			gui.set_position(word.node,  settings.image_pixel_grid_snap and round(pos) or pos)
		else
			local pos = vmath.vector3(position.x,position.y,0)
...
local function create_box_node(word)
	...
	if(word.image.anim == "icon_sber_mic")then
		word.size =  word.size * 0.4
		gui.set_scale(node,vmath.vector3(word.size))
	end
...
local function create_node(word, parent, font, node, metrics)
	...
	gui.set_pivot(node, M.DEFAULT_ALIGN or gui.PIVOT_NW)
...
function M.create(text, font, settings)
	...
	for i = 1, word_count do
        ...
        if(word_metrics.pad_top)then
            line_pad.top =  word_metrics.pad_top
        end
		...
		if overflow and not word.nobr then
			...
			position.y = settings.position.y - text_metrics.height - line_pad.top
			...
			text_metrics.height = text_metrics.height + (line_height * settings.line_spacing) + line_pad.top
			line_width = word_metrics.total_width
			line_height = word_metrics.height + line_pad.top
            line_pad.top = 0
